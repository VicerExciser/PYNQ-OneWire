...
class TemperatureController():
	...
	def __init__(self, sensor_type=ds18b20.DS18B20):
		...
		self.sensors = []
		for sensor in self.ow_bus.search(self.sensor_class, self.sensor_class.rom_cmds['SRCH_ROM']):
...
## ---------------------------------------------------------------------------------------------
	...
	def match_rom(self, rom_lo, rom_hi):
		...
		"""
		OneWire.bram_write(addr_map['CMD_ADDR'], self.sensor_class.rom_cmds['MTCH_ROM'])
		OneWire.bram_write(addr_map['WRS_ADDR'], self.sensor_class.TRANSMIT_BITS)
...
## ---------------------------------------------------------------------------------------------
	...
	def convert_temp(self):
		...
		"""
		OneWire.bram_write(addr_map['CMD_ADDR'], self.sensor_class.func_cmds['CONVT_TEMP'])
		OneWire.bram_write(addr_map['CON_ADDR'], bus_cmds['EXEC_W_PULLUP'])
		time.sleep(self.sensor_class.T_CONV)
		r_status = OneWire.bram_read(addr_map['STA_ADDR'])
		return r_status == 0x8
...
## ---------------------------------------------------------------------------------------------
	...
	def read_scratch(self):
		...
		"""
		OneWire.bram_write(addr_map['CMD_ADDR'], self.sensor_class.func_cmds['SCRATCH_RD'])
		OneWire.bram_write(addr_map['RDS_ADDR'], self.sensor_class.SCRATCH_RD_SIZE)
